...
1. **Print the rule** — shows the chat key (or `#id`) and model name.
2. **Prompt loop** — uses `prompt_toolkit.PromptSession` with `InMemoryHistory`, custom key bindings, and multiline mode.
3. **Command dispatch** — if the input starts with `/`, it is matched against `_REPL_COMMANDS`. Unknown `/` words trigger a confirmation prompt before being forwarded to the AI.
4. **AI turn** — calls `core.chat.send_message()`, renders a spinner while waiting, then prints the completed response as a Rich panel with Markdown.
5. **Auto-key** — after the first successful exchange, calls `core.context.generate_chat_key()` and stores the result via `database.update_chat_key()`.
...
| `render_chat_list(...)`  | Renders the chat list table (Key col wraps)       |
